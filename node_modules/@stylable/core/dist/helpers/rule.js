...
function createSubsetAst(root, selectorPrefix, mixinTarget, isRoot = false, getCustomSelector, isNestedInMixin = false) {
    ...
    root.nodes.forEach((node) => {
        if (node.type === 'decl') {
            ...
            let ast = isRoot
                ? (0, selector_1.unlockNestedSelector)(prefixSelectorList, selectorAst, true).ast
                : selectorAst;
            ...
            if (node.name === 'media' ||
                ...
                node.name === 'layer' ||
                node.name === 'container') {
                let unlockSelector = node.name === 'st-unlock' ? node.params : '';
                let atruleHasMixin = isNestedInMixin || false;
                if (unlockSelector) {
                    const ast = (0, selector_1.parseSelectorWithCache)(unlockSelector, { clone: true });
                    const matchesSelectors = isRoot
                        ? ast
                        : ast.filter((node) => containsPrefix(node));
                    if (matchesSelectors.length) {
                        atruleHasMixin = true;
                        unlockSelector = (0, selector_1.stringifySelector)(matchesSelectors.map((selectorNode) => {
                            if (!isRoot) {
                                selectorNode = fixChunkOrdering(selectorNode, prefixType);
                            }
                            replaceTargetWithMixinAnchor(selectorNode, prefixType);
                ...
                const atRuleSubset = createSubsetAst(node, selectorPrefix, postcss.atRule({
                    params: unlockSelector || node.params,
                    name: node.name,
