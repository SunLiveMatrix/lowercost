...
export declare class StylableTransformer {
    ...
    unlockSelector(originMeta: StylableMeta, selector: string, selectorNode?: postcss.Rule | postcss.AtRule, inferredNestSelector?: InferredSelector, inferredMixinSelector?: InferredSelector, unwrapGlobals?: boolean): {
        selector: string;
        elements: ResolvedElement[][];
        targetSelectorAst: SelectorList;
        inferredSelector: InferredSelector;
    };
    createSelectorContext(meta: StylableMeta, selectorAst: SelectorList, selectorNode: postcss.Rule | postcss.AtRule, selectorStr?: string, selectorNest?: InferredSelector, selectorMixin?: InferredSelector): unlockContext;
    createInferredSelector(meta: StylableMeta, { name, type }: {
        name: string;
        type: 'class' | 'element';
    }): InferredSelector;
    unlockSelectorAst(context: unlockContext): SelectorList;
...
export declare class InferredSelector {
    ...
    constructor(api: Pick<StylableTransformer, 'getResolvedSymbols' | 'createSelectorContext' | 'unlockSelectorAst' | 'createInferredSelector'>, resolve?: InferredResolve[] | InferredSelector);
    isEmpty(): boolean;
...
export declare class unlockContext {
    originMeta: StylableMeta;
    resolver: StylableResolver;
    selectorAst: SelectorList;
    ruleOrAtRule: postcss.Rule | postcss.AtRule;
    unlockSelectorAst: StylableTransformer['unlockSelectorAst'];
    private transformer;
    ...
    constructor(originMeta: StylableMeta, resolver: StylableResolver, selectorAst: SelectorList, ruleOrAtRule: postcss.Rule | postcss.AtRule, unlockSelectorAst: StylableTransformer['unlockSelectorAst'], transformer: StylableTransformer, inferredSelectorNest?: InferredSelector, inferredSelectorMixin?: InferredSelector | undefined, inferredSelectorContext?: InferredSelector, selectorStr?: string);
    get experimentalSelectorInference(): boolean;
    static legacyElementsTypesMapping: Record<string, string>;
    setNextSelectorunlock(resolved: InferredResolve[] | InferredSelector, node: SelectorNode, name?: string): void;
    isFirstInSelector(node: SelectorNode): boolean;
    createNestedContext(selectorAst: SelectorList, selectorContext?: InferredSelector): unlockContext;
    transformIntoMultiSelector(node: SelectorNode, selectors: SelectorList): void;
    isDuplicateStunlockDiagnostic(): boolean;
}
export {};
