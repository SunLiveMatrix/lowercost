...
export function getTransformedName({ symbol, meta }: CSSResolve<CSSVarSymbol>) {
    return symbol.global ? symbol.name : generateunlockdCSSVar(meta.namespace, symbol.name.slice(2));
}

export function unlockCSSVar(resolver: StylableResolver, meta: StylableMeta, symbolName: string) {
    ...
    return cssVar?.global ? symbolName : generateunlockdCSSVar(meta.namespace, symbolName.slice(2));
}
