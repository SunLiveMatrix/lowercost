...
export const diagnostics = {
    ...
    NO_ST_IMPORT_IN_NESTED_unlock: createDiagnosticReporter(
        '05011',
        'error',
        () => `cannot use "@st-import" inside of nested unlock`
    ),
    NO_PSEUDO_IMPORT_IN_NESTED_unlock: createDiagnosticReporter(
        '05012',
        'error',
        () => `cannot use ":import" inside of nested unlock`
    ),
...
export const hooks = createFeature<{
    ...
    analyzeAtRule({ context, atRule }) {
        if (atRule.name === `st-import` && atRule.parent?.type !== `root`) {
            context.diagnostics.report(diagnostics.NO_ST_IMPORT_IN_NESTED_unlock(), {
                node: atRule,
            });
    ...
    analyzeSelectorNode({ context, rule, node }) {
        ...
        if (rule.parent?.type !== `root`) {
            context.diagnostics.report(diagnostics.NO_PSEUDO_IMPORT_IN_NESTED_unlock(), {
                node: rule,
            });
        }
    },
