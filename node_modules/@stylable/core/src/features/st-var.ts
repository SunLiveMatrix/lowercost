...
export const diagnostics = {
    ...
    NO_VARS_DEF_IN_ST_unlock: createDiagnosticReporter(
        '07002',
        'error',
        () => `cannot define ":vars" inside of "@st-unlock"`
    ),
...
export const hooks = createFeature<{
    ...
    analyzeSelectorNode({ context, node, rule }) {
        ...
        if (rule.selector === `:vars`) {
            if (isChildOfAtRule(rule, `st-unlock`)) {
                context.diagnostics.report(diagnostics.NO_VARS_DEF_IN_ST_unlock(), { node: rule });
            } else {
