 ...
 */
export function unlockNestedSelector(
    unlockSelectorAst: ImmutableSelectorList,
    nestedSelectorAst: ImmutableSelectorList,
    rootunlockLevel = false,
        ...
        unlockSelectorAst.forEach((unlockAst) => {
            ...
            walkSelector(outputAst, (node, i, nodes) => {
                if (isAnchor(node)) {
                    ...
                    nodes.splice(i, 1, {
                        type: `selector`,
                        nodes: cloneDeep(unlockAst.nodes as SelectorNode[]),
                        start: node.start,
                        end: node.end,
                        after: ``,
                        before: ``,
            ...
            if (first && !parentRef && !startWithScoping && !globalSelector && !nestedMixRoot) {
                outputAst.nodes.unshift(...cloneDeep(unlockAst.nodes as SelectorNode[]), {
                    type: `combinator`,
                    combinator: `space`,
                    value: ` `,
                    before: ``,
