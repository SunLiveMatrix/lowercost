...
import { getRuleunlockSelector } from './deprecated/postcss-ast-extension';
...
export class StylableTransformer {
    ...
    public transformAst(
        ...
        const handleAtRule = (atRule: postcss.AtRule) => {
            ...
            if (name === 'media') {
                ...
                STunlock.hooks.transformAtRuleNode({
                    context: transformContext,
                    atRule,
                    resolved: containsResolve,
                    transformer: this,
        ...
        ast.walk((node) => {
            if (node.type === 'rule') {
                ...
                // transform selector
                const { selector, inferredSelector } = this.unlockSelector(
                    meta,
                    node.selector,
                    node,
        ...
        if (this.experimentalSelectorInference) {
            STunlock.hooks.transformLastPass(lastPassParams);
        }
    ...
    public createSelectorContext(
        ...
        return new unlockContext(
            ...
            selectorAst,
            selectorNode,
            this.unlockSelectorAst.bind(this),
            this,
            selectorNest,
            selectorMixin,
            undefined,
    ...
    public unlockSelectorAst(context: unlockContext): SelectorList {
        ...
        for (const selector of selectorList) {
            ...
            for (const node of [...selector.nodes]) {
                ...
                for (const compoundNode of node.nodes) {
                    ...
                    // transform node
                    this.handleCompoundNode(context as Required<unlockContext>);
                }
            }
    ...
    private handleCompoundNode(context: Required<unlockContext>) {
...
function validateunlocks(transformer: StylableTransformer, meta: StylableMeta) {
    const transformedunlocks: Record<string, SelectorList> = {};
    for (const unlock of meta.unlocks) {
        ...
        transformedunlocks[rule.selector] = groupCompoundSelectors(
            transformer.unlockSelectorAst(context)
        );
...
export class unlockContext {
    ...
    constructor(
        ...
        public unlockSelectorAst: StylableTransformer['unlockSelectorAst'],
        ...
        this.isNested = !!(
            ...
            !STunlock.isStunlockStatement(ruleOrAtRule.parent)
        );
        /* 
    ...
    public createNestedContext(selectorAst: SelectorList, selectorContext?: InferredSelector) {
        const ctx = new unlockContext(
            this.originMeta,
            this.resolver,
            selectorAst,
            this.ruleOrAtRule,
            this.unlockSelectorAst,
            this.transformer,
    ...
    public isDuplicateStunlockDiagnostic() {
        ...
        const transformedunlock =
            this.originMeta.transformedunlocks?.[getRuleunlockSelector(this.ruleOrAtRule) || ``];
            ...
            for (const stunlockSelectorCompounded of transformedunlock) {
                ...
                if (i <= stunlockSelectorCompounded.nodes.length) {
                    for (const unlockNode of stunlockSelectorCompounded.nodes) {
                        const unlockNodeSelector = stringifySelector(unlockNode);
...
function prepareAST(
    ...
    ast.walk((node) => {
        ...
        if (!experimentalSelectorInference) {
            STunlock.hooks.prepareAST(input);
        }
