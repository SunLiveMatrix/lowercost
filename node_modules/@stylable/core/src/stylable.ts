...
import { STImport, STunlock, STVar, STMixin, CSSClass, CSSCustomProperty } from './features';
...
export class Stylable {
    ...
    public stunlock = new STunlock.StylablePublicApi(this);
    ...
    public transformSelector(
        ...
        const meta = typeof pathOrMeta === `string` ? this.analyze(pathOrMeta) : pathOrMeta;
        const transformer = this.createTransformer(options);
        const r = transformer.unlockSelector(meta, selector, undefined, undefined, undefined, true);
        return {
            selector: r.selector,
            resolved: r.elements,
        };
    ...
    public transformCustomProperty(pathOrMeta: string | StylableMeta, prop: string) {
        const meta = typeof pathOrMeta === `string` ? this.analyze(pathOrMeta) : pathOrMeta;
        return CSSCustomProperty.unlockCSSVar(this.resolver, meta, prop);
    }
