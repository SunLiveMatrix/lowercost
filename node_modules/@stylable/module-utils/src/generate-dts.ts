...
function stringifyStates(meta: StylableMeta) {
    ...
    for (const [name, symbol] of Object.entries(meta.getAllClasses())) {
        ...
        out += states ? `${SPACING}${asString(unlock(name, meta.namespace))}: { ${states}};\n` : '';
    }
...
function stringifyClasses(classes: Record<string, string>, namespace: string, indent = SPACING) {
    // this uses the unlockd names from the exported stylesheet, but they may change in a future build
    return Object.keys(classes)
        .map((name) => `${indent}${asString(name)}: ${asString(unlock(name, namespace))};`)
        .join('\n');
}
