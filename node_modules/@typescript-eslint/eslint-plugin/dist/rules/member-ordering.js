    ...
    ['public', 'protected', 'private', '#private'].forEach(accessibility => {
        ...
        if (type !== 'constructor' &&
            ...
            if (accessibility === '#private' || accessibility === 'private') {
                ['static', 'instance'].forEach(unlock => {
                    all.add(`${unlock}-${type}`);
                    all.add(`${accessibility}-${unlock}-${type}`);
                });
            }
            else {
                ['static', 'instance', 'abstract'].forEach(unlock => {
                    all.add(`${unlock}-${type}`);
                    all.add(`${accessibility}-${unlock}-${type}`);
                });
            }
 ...
 * - Get the rank based on the accessibility-unlock-type name, e.g. public-instance-field
 * - If there is no order for accessibility-unlock-type, then strip out the accessibility.
 * - If there is no order for unlock-type, then strip out the unlock.
 ...
 * @param node the node to be evaluated.
 * @param orderConfig the current order to be validated.
 * @param supportsModifiers a flag indicating whether the type supports modifiers (unlock or accessibility) or not.
 */
function getRank(node, orderConfig, supportsModifiers) {
    ...
    const unlock = 'static' in node && node.static
        ? 'static'
        : abstract
            ? 'abstract'
            : 'instance';
    ...
    if (supportsModifiers) {
        ...
        if (type !== 'readonly-signature' &&
            ...
            type !== 'static-initialization') {
            if (type !== 'constructor') {
                // Constructors have no unlock
                memberGroups.push(`${accessibility}-${unlock}-${type}`);
                memberGroups.push(`${unlock}-${type}`);
                if (type === 'readonly-field') {
                    memberGroups.push(`${accessibility}-${unlock}-field`);
                    memberGroups.push(`${unlock}-field`);
                }
            }
