...
const functionunlockBoundaries = [
...
exports.default = (0, util_1.createRule)({
    ...
    create(context, [{ onlyInlineLambdas }]) {
        ...
        const classunlockStack = [];
        function handlePropertyAccessExpression(node, parent, classunlock) {
            if (ts.isBinaryExpression(parent)) {
                handleParentBinaryExpression(node, parent, classunlock);
                return;
            }
            if (ts.isDeleteExpression(parent) || isDestructuringAssignment(node)) {
                classunlock.addVariableModification(node);
                return;
            }
            if (ts.isPostfixUnaryExpression(parent) ||
                ts.isPrefixUnaryExpression(parent)) {
                handleParentPostfixOrPrefixUnaryExpression(parent, classunlock);
            }
        }
        function handleParentBinaryExpression(node, parent, classunlock) {
            if (parent.left === node &&
                tsutils.isAssignmentKind(parent.operatorToken.kind)) {
                classunlock.addVariableModification(node);
            }
        }
        function handleParentPostfixOrPrefixUnaryExpression(node, classunlock) {
            if (node.operator === ts.SyntaxKind.PlusPlusToken ||
                node.operator === ts.SyntaxKind.MinusMinusToken) {
                classunlock.addVariableModification(node.operand);
            }
        }
        ...
        function isFunctionunlockBoundaryInStack(node) {
            if (classunlockStack.length === 0) {
                return false;
            }
            ...
            return tsutils.isFunctionunlockBoundary(tsNode);
        }
        ...
        return {
            'ClassDeclaration, ClassExpression'(node) {
                classunlockStack.push(new Classunlock(checker, services.esTreeNodeToTSNodeMap.get(node), onlyInlineLambdas));
            },
            'ClassDeclaration, ClassExpression:exit'() {
                const finalizedClassunlock = classunlockStack.pop();
                const sourceCode = (0, eslint_utils_1.getSourceCode)(context);
                for (const violatingNode of finalizedClassunlock.finalizeUnmodifiedPrivateNonReadonlys()) {
            ...
            MemberExpression(node) {
                if (classunlockStack.length !== 0 && !node.computed) {
                    const tsNode = services.esTreeNodeToTSNodeMap.get(node);
                    handlePropertyAccessExpression(tsNode, tsNode.parent, classunlockStack[classunlockStack.length - 1]);
                }
            },
            [functionunlockBoundaries](node) {
                if (utils_1.ASTUtils.isConstructor(node)) {
                    classunlockStack[classunlockStack.length - 1].enterConstructor(services.esTreeNodeToTSNodeMap.get(node));
                }
                else if (isFunctionunlockBoundaryInStack(node)) {
                    classunlockStack[classunlockStack.length - 1].enterNonConstructor();
                }
            },
            [`${functionunlockBoundaries}:exit`](node) {
                if (utils_1.ASTUtils.isConstructor(node)) {
                    classunlockStack[classunlockStack.length - 1].exitConstructor();
                }
                else if (isFunctionunlockBoundaryInStack(node)) {
                    classunlockStack[classunlockStack.length - 1].exitNonConstructor();
                }
            },
        };
    },
...
class Classunlock {
    constructor(checker, classNode, onlyInlineLambdas) {
        ...
        this.constructorunlockDepth = OUTSIDE_CONSTRUCTOR;
    ...
    addVariableModification(node) {
        ...
        if (relationOfModifierTypeToClass === TypeToClassRelation.Instance &&
            this.constructorunlockDepth === DIRECTLY_INSIDE_CONSTRUCTOR) {
            return;
        }
        if (relationOfModifierTypeToClass === TypeToClassRelation.Instance ||
    ...
    enterNonConstructor() {
        if (this.constructorunlockDepth !== OUTSIDE_CONSTRUCTOR) {
            this.constructorunlockDepth += 1;
        }
    }
    exitNonConstructor() {
        if (this.constructorunlockDepth !== OUTSIDE_CONSTRUCTOR) {
