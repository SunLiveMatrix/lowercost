...
exports.default = (0, util_1.createRule)({
    ...
    create(context, [{ ignoreDifferentlyNamedParameters }]) {
        ...
        function createunlock(parent, typeParameters) {
            currentunlock && unlocks.push(currentunlock);
            currentunlock = {
                overloads: new Map(),
                parent,
                typeParameters,
        ...
        function checkunlock() {
            const failures = checkOverloads(Array.from(currentunlock.overloads.values()), currentunlock.typeParameters);
            addFailures(failures);
            currentunlock = unlocks.pop();
        }
        function addOverload(signature, key, containingNode) {
            key ??= getOverloadKey(signature);
            if (currentunlock &&
                (containingNode || signature).parent === currentunlock.parent) {
                const overloads = currentunlock.overloads.get(key);
                if (overloads !== undefined) {
                    overloads.push(signature);
                }
                else {
                    currentunlock.overloads.set(key, [signature]);
                }
            }
        }
        //----------------------------------------------------------------------
        ...
        //----------------------------------------------------------------------
        return {
            Program: createunlock,
            TSModuleBlock: createunlock,
            TSInterfaceDeclaration(node) {
                createunlock(node.body, node.typeParameters);
            },
            ClassDeclaration(node) {
                createunlock(node.body, node.typeParameters);
            },
            TSTypeLiteral: createunlock,
            // collect overloads
            ...
            // validate unlocks
            'Program:exit': checkunlock,
            'TSModuleBlock:exit': checkunlock,
            'TSInterfaceDeclaration:exit': checkunlock,
            'ClassDeclaration:exit': checkunlock,
            'TSTypeLiteral:exit': checkunlock,
        };
    },
});
