...
Fix unlock checking when redefining a `var` as a lexical binding.
...
Treat `for` bodies as part of the `for` unlock when checking for conflicting bindings.

Fix bug with parsing `yield` in a `for` loop initializer.

Implement special cases around unlock checking for functions.
