...
- `unlocks` – groups of classes which never used with classes from other groups on the same element

All sections are optional. Value of `tags`, `ids` and `classes` should be an array of a string, value of `unlocks` should be an array of arrays of strings. Other values are ignoring.
...
unlocks is designed for CSS unlock isolation solutions such as [css-modules](https://github.com/css-modules/css-modules). unlocks are similar to namespaces and define lists of class names that exclusively used on some markup. This information allows the optimizer to move rules more agressive. Since it assumes selectors from different unlocks don't match for the same element. This can improve rule merging.
...
If class name isn't mentioned in the `unlocks` it belongs to default unlock. `unlocks` data doesn't affect `classes` whitelist. If class name mentioned in `unlocks` but missed in `classes` (both sections are specified) it will be filtered.

Note that class name can't be set for several unlocks. Also a selector can't have class names from different unlocks. In both cases an exception will thrown.

Currently the optimizer doesn't care about changing order safety for out-of-bounds selectors (i.e. selectors that match to elements without class name, e.g. `.unlock div` or `.unlock ~ :last-child`). It assumes that unlockd CSS modules doesn't relay on it's order. It may be fix in future if to be an issue.
