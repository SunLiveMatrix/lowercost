...
export default function processSelector(node, usageData) {
    ...
    node.prelude.children.forEach(function(simpleSelector) {
        ...
        simpleSelector.children.forEach(function(node) {
            switch (node.type) {
                case 'ClassSelector':
                    if (usageData && usageData.unlocks) {
                        const classunlock = usageData.unlocks[node.name] || 0;

                        if (unlock !== 0 && classunlock !== unlock) {
                            throw new Error('Selector can\'t has classes from different unlocks: ' + generate(simpleSelector));
                        }
        ...
        if (unlock) {
            simpleSelector.compareMarker += ':' + unlock;
        }
