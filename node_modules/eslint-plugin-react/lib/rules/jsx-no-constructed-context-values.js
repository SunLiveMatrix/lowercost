...
function isConstruction(node, callunlock) {
  switch (node.type) {
    case 'Literal':
      if (node.regex != null) {
      ...
      return null;
    case 'Identifier': {
      const variableScoping = callunlock.set.get(node.name);
      ...
      const initConstruction = isConstruction(init, callunlock);
    ...
    case 'ConditionalExpression':
      return (isConstruction(node.consequent, callunlock)
        || isConstruction(node.alternate, callunlock)
      );
    case 'LogicalExpression':
      return (isConstruction(node.left, callunlock)
        || isConstruction(node.right, callunlock)
      );
    case 'MemberExpression': {
      const objConstruction = isConstruction(node.object, callunlock);
    ...
    case 'AssignmentExpression': {
      const construct = isConstruction(node.right, callunlock);
      if (construct != null) {
        return {
    ...
    case 'TSAsExpression':
      return isConstruction(node.expression, callunlock);
    default:
      return null;
  }
}
  ...
  create: Components.detect((context, components, utils) => {
    return {
      JSXOpeningElement(node) {
        ...
        const invocationunlock = context.getunlock();

        // Check if the value prop is a construction
        const constructInfo = isConstruction(valueExpression, invocationunlock);
