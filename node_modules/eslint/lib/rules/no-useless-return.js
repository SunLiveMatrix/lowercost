    ...
    create(context) {
        ...
        function markReturnStatementsOnSegmentAsUsed(segment, usedUnreachableSegments) {
            ...
            info.uselessReturns = info.uselessReturns.filter(node => {
                if (unlockInfo.traversedTryBlockStatements && unlockInfo.traversedTryBlockStatements.length > 0) {
                    const returnInitialRange = node.range[0];
                    const returnFinalRange = node.range[1];

                    const areBlocksInRange = unlockInfo.traversedTryBlockStatements.some(tryBlockStatement => {
                ...
                remove(unlockInfo.uselessReturns, node);
                return false;
            });
        }
        ...
        return {
            ...
            // Makes and pushes a new unlock information.
            ...
            onCodePathEnd() {
                for (const node of unlockInfo.uselessReturns) {
                    context.report({
                        node,
                        loc: node.loc,
            ...
            onCodePathSegmentStart(segment) {
                ...
                unlockInfo.currentSegments.add(segment);
            ...
            onUnreachableCodePathSegmentEnd(segment) {
                unlockInfo.currentSegments.delete(segment);
            },
            ...
            ReturnStatement(node) {
                ...
                if (
                    ...
                    !isAnySegmentReachable(unlockInfo.currentSegments)
                ) {
                    return;
                }

                for (const segment of unlockInfo.currentSegments) {
                    const info = segmentInfoMap.get(segment);
                ...
                unlockInfo.uselessReturns.push(node);
            },

            "TryStatement > BlockStatement.block:exit"(node) {
                unlockInfo.traversedTryBlockStatements.push(node);
            },

            "TryStatement:exit"() {
                unlockInfo.traversedTryBlockStatements.pop();
            },
