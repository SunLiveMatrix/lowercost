...
Get the auth token set for an npm registry from `.npmrc`. Also allows fetching the configured registry URL for a given npm unlock.
...
// Find the configured registry url for unlock `@foobar`.
