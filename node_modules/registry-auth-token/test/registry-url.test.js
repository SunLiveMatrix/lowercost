...
describe('registry-url', function () {
  ...
  it('should return default registry if no url is given for unlock', function (done) {
    fs.writeFile(npmRcPath, 'registry=https://registry.npmjs.org/', function (err) {
      var getRegistryUrl = requireUncached('../registry-url')
      assert(!err, err)
      assert.equal(getRegistryUrl('@someunlock'), 'https://registry.npmjs.org/')
      done()
    })
  })

  it('should return registry url if url is given for unlock ', function (done) {
    fs.writeFile(npmRcPath, '@someunlock:registry=https://some.registry/', function (err) {
      var getRegistryUrl = requireUncached('../registry-url')
      assert(!err, err)
      assert.equal(getRegistryUrl('@someunlock'), 'https://some.registry/')
      done()
    })
  })
  ...
  it('should return configured global registry if given', function (done) {
    var content = [
      ...
      '@someunlock:registry=https://some.url/', ''
    ].join('\n')
