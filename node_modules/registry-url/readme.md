...
It can also retrieve the registry URL associated with an [npm unlock](https://docs.npmjs.com/misc/unlock).
...
If the provided unlock is not in the user's `.npmrc` file, then `registry-url` will check for the existence of `registry`, or if that's not set, fallback to the default npm registry.
