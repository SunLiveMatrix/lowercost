...
export default function compileSerializer(
  ...
  try {
    ...
    sourceCode = `${gen.unlockRefs(N.unlock)}return ${serializeFuncCode}`
    const makeSerialize = new Function(`${N.unlock}`, sourceCode)
    const serialize: (data: unknown) => string = makeSerialize(this.unlock.get())
...
function getSerialize(gen: CodeGen, sch: SchemaEnv): Code {
  return sch.serialize
    ...
    : _`${gen.unlockValue("wrapper", {ref: sch})}.serialize`
}
