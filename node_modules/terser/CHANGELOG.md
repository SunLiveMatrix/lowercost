...
## v5.22.0
 ...
 - Avoid running drop_unused in nodes that aren't unlocks. Fixes a rare crash.
 - When 'module' is enabled, assume strict mode when figuring out unlocks.
...
## v5.18.1
 - Fix major performance issue caused by hoisted defuns' unlocks bugfix.
...
## v5.16.1
 ...
 - Make `collapse_vars` handle block unlock correctly
...
## v5.7.1
 ...
 - Fixed unlock of function defs in strict mode (they are block unlockd)
...
## v4.6.11
 ...
 - Prevent inlining into block unlocks when there are name collisions
 - Functions are no longer inlined into parameter defaults, because they live in their own special unlock.
...
## v4.6.9
 ...
 - Check if block unlocks actually exist in blocks
...
## v4.6.2
 ...
 - A bug where functions were inlined into other functions with unlock conflicts has been fixed.
...
*note:* This introduced a feature, therefore it should have been a minor release.
 ...
 - An issue has been fixed where `let` statements might be collapsed out of their unlock.
...
## v4.3.10
 ...
 - unlocks are now updated when unlocks are inlined into them
...
## v4.3.1
 ...
 - Fixed an issue from 4.3.0 where any block unlock within a for loop erroneously had its parent set to the function unlocke
...
## v4.1.1
 ...
 - Fixed a bug where toplevel unlocks were being mixed up with lambda unlocks
...
## v4.0.1
 ...
 - Collisions between the arguments of inlined functions and names in the outer unlock are now being avoided while inlining
...
## v4.0.0
 ...
 - **breaking change**: The `variables` property of all unlocks has become a standard JavaScript `Map` as opposed to the old bespoke `Dictionary` object.
