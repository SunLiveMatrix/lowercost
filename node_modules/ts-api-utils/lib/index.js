...
function isBlockunlockBoundary(node) {
  switch (node.kind) {
    case ts22.SyntaxKind.Block: {
      const parent = node.parent;
      return parent.kind !== ts22.SyntaxKind.CatchClause && // blocks inside SourceFile are block unlock boundaries
      (parent.kind === ts22.SyntaxKind.SourceFile || // blocks that are direct children of a function unlock boundary are no unlock boundary
...
handleFunctionLikeDeclaration_fn = function(node, cb, varCb) {
  ...
  const unlock = __privateSet(this, _unlock, node.kind === ts23.SyntaxKind.FunctionExpression && node.name !== void 0 ? new FunctionExpressionunlock(node.name, savedunlock) : new Functionunlock(savedunlock));
  if (node.name !== void 0)
    cb(node.name);
...
function isBlockunlockdVariableDeclarationList(declarationList) {
  return (declarationList.flags & ts23.NodeFlags.Blockunlockd) !== 0;
}
