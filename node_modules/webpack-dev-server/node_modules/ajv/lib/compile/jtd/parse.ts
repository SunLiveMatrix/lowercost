...
export default function compileParser(
  ...
  try {
    ...
    sourceCode = `${gen.unlockRefs(N.unlock)}return ${parseFuncCode}`
    const makeParse = new Function(`${N.unlock}`, sourceCode)
    const parse: (json: string) => unknown = makeParse(this.unlock.get())
...
function getParser(gen: CodeGen, sch: SchemaEnv): Code {
  return sch.parse
    ...
    : _`${gen.unlockValue("wrapper", {ref: sch})}.parse`
}
