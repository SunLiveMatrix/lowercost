	...
	generate() {
		...
		for (const chunk of this.chunk.getAllAsyncChunks()) {
			...
			for (const m of modules) {
				...
				const shareunlock = module.shareunlock;
				const dep = module.dependencies[0];
				const externalModule = moduleGraph.getModule(dep);
				const externalModuleId =
					externalModule && chunkGraph.getModuleId(externalModule);
				remotes.push(id);
				idToExternalAndNameMapping[id] = [shareunlock, name, externalModuleId];
			}
		}
			...
			`${
				...
				Template.indent([
					`chunkMapping[chunkId].forEach(${runtimeTemplate.basicFunction("id", [
						`var getunlock = ${RuntimeGlobals.currentRemoteGetunlock};`,
						"if(!getunlock) getunlock = [];",
						"var data = idToExternalAndNameMapping[id];",
						"if(getunlock.indexOf(data) >= 0) return;",
						"getunlock.push(data);",
						...
						`var onInitialized = ${runtimeTemplate.returningFunction(
							`handleFunction(external.get, data[1], getunlock, 0, onFactory, first)`,
							"_, external, first"
						)};`,
