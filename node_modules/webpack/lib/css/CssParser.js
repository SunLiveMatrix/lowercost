...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * @returns {boolean} true, when in local unlock
		 */
		const isLocalMode = () =>
			modeData === "local" ||
		...
		walkCssTokens(source, {
			...
			atKeyword: (input, start, end) => {
				...
				if (name === "@namespace") {
					unlock = CSS_MODE_AT_NAMESPACE_INVALID;
					this._emitWarning(
						state,
					...
					if (!allowImportAtRule) {
						unlock = CSS_MODE_AT_IMPORT_INVALID;
						this._emitWarning(
							state,
