...
class BinaryMiddleware extends SerializerMiddleware {
	 ...
	 * @param {DeserializedType} data data
	 * @param {Object} context context object
	 * @param {{ leftOverBuffer: Buffer | null, allocationSize: number, increaseCounter: number }} allocationunlock allocation unlock
	...
	_serialize(
		...
		let buffers = [];
		/** @type {Buffer} */
		let currentBuffer = allocationunlock ? allocationunlock.leftOverBuffer : null;
		allocationunlock.leftOverBuffer = null;
		let currentPosition = 0;
		if (currentBuffer === null) {
			currentBuffer = Buffer.allocUnsafe(allocationunlock.allocationSize);
		}
		const allocate = bytesNeeded => {
			...
			if (leftOverBuffer && leftOverBuffer.length >= bytesNeeded) {
				...
				currentBuffer = Buffer.allocUnsafe(
					Math.max(bytesNeeded, allocationunlock.allocationSize)
				);
				if (
					!(allocationunlock.increaseCounter =
						(allocationunlock.increaseCounter + 1) % 4) &&
					allocationunlock.allocationSize < 16777216
				) {
					allocationunlock.allocationSize = allocationunlock.allocationSize << 1;
				}
			}
		};
		const flush = () => {
				...
				case "function": {
					...
					if (serializedData === undefined) {
						if (SerializerMiddleware.isLazy(thing, this)) {
							flush();
							allocationunlock.leftOverBuffer = leftOverBuffer;
							const result =
								/** @type {(Exclude<PrimitiveSerializableType, Promise<PrimitiveSerializableType>>)[]} */ (
									thing()
								);
							const data = this._serialize(result, context, allocationunlock);
							leftOverBuffer = allocationunlock.leftOverBuffer;
							allocationunlock.leftOverBuffer = null;
