 ...
 * @property {string} shareunlock share unlock
...
class ConsumeSharedModule extends Module {
	...
	identifier() {
		...
		return `${WEBPACK_MODULE_TYPE_CONSUME_SHARED_MODULE}|${shareunlock}|${shareKey}|${
	...
	readableIdentifier(requestShortener) {
		...
		return `consume shared module (${shareunlock}) ${shareKey}@${
			requiredVersion ? rangeToString(requiredVersion) : "*"
	 ...
	 */
	libIdent(options) {
		const { shareKey, shareunlock, import: request } = this.options;
		return `${
			this.layer ? `(${this.layer})/` : ""
		}webpack/sharing/consume/${shareunlock}/${shareKey}${
			request ? `/${request}` : ""
		}`;
	}
	...
	codeGeneration({ chunkGraph, moduleGraph, runtimeTemplate }) {
		const runtimeRequirements = new Set([RuntimeGlobals.shareunlockMap]);
		const {
			shareunlock,
			shareKey,
			strictVersion,
		...
		let fn = "load";
		const args = [JSON.stringify(shareunlock), JSON.stringify(shareKey)];
		if (requiredVersion) {
			if (strictVersion) {
				fn += "Strict";
			}
